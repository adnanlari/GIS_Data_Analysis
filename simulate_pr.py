		...
		if len(row)!=1:
			...
			if int(current_datarate)!=0:
				...
				for ind in range(len(list_of_files)):
					...
					dc['m_id']=list_of_files[ind][1]
					dc['m_type']=list_of_files[ind][2]
					dc['size']=list_of_files[ind][3]
					dc['tile']=list_of_files[ind][4]
					...
					if(time_req<=avail_time):
						MySeriesHelper(**dc)
						avail_time= avail_time - time_req
					print(list_of_files[ind][1],"is entered in DB")
