						...
						for rs in genread:
							if len(rs)!=1:
								if int(rs[0])<=int(row[0]):
									query_st = "select m_id from sDB2 where m_id= '" + rs[6].strip() + "'"
									print("Query: ",query_st)
									res = myclient.query(query_st)
									res = list(res.get_points(measurement='sDB2'))
									...
									if len(res)==0:
										...
										if(time_req<=avail_time):
											...
											dc['tile']=rs[1]
											MySeriesHelper(**dc)
											print("Entered in DB  gen",rs[6].strip(),avail_time)
										else:
											print("not enough time ",avail_time,time_req)
									else:
										print("File ",rs[6].strip(),"is in DB")
								else:
				...
				if col.is_file():
					...
					if colread is not None:
						...
						for rs in colread:
							if len(rs)!=1:
								if int(rs[0])<=int(row[0]):
									query_st = "select m_id from sDB2 where m_id= '" + rs[2].strip() + "'"
									res = myclient.query(query_st)
									res = list(res.get_points(measurement='sDB2'))
